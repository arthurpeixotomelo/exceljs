...
class WorksheetWriter {
  ...
  _writeSheetProperties(xmlBuf, properties, pageSetup) {
    ...
    xmlBuf += xform.sheetProperties.toXml(sheetPropertiesModel)
  }
  ...
  _writeSheetFormatProperties(xmlBuf, properties) {
    ...
    xmlBuf += xform.sheetFormatProperties.toXml(sheetFormatPropertiesModel);
  }
  ...
  _writeOpenWorksheet() {
    ...
    xmlBuffer += '<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"' +
    ...
    xmlBuffer += xform.sheetViews.toXml(this.views);
  ...
  _writeMergeCells() {
    if (this._merges.length) {
      let xml = `<mergeCells count="${this._merges.length}">`;
      this._merges.forEach(merge => {
  ...
  _writeLegacyData() {
    if (this.hasComments) {
      const xml = `<legacyDrawing r:id="${this._sheetCommentsWriter.vmlRelId}"/>`;
      this.stream.write(xml);
    }
  }
