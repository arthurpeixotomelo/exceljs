...
const makeTestXlsxFileName = (basename = 'wb.test') => `./tests/out/${basename}.${Date.now()}.${Math.random().toString(36).slice(2)}.xlsx`;
const IMAGE_FILENAME = resolve(__dirname, '../data/image.png');
...
describe('WorkbookWriter', () => {
  it('creates sheets with correct names', () => {
    ...
    // close the zip and the stream to avoid leaving open handles that keep test runner alive
    if (wb && wb.zip && wb.zip.finalize) {
      try {
        wb.zip.finalize();
      } catch (err) {
        // ignore any errors finalizing the zip (we're only ensuring no open handles)
      }
    }
    if (wb && wb.stream && wb.stream.end) wb.stream.end();
  });

  describe('Serialise', () => {
    it('xlsx file', () => {
      const options = {
        filename: makeTestXlsxFileName(),
        useStyles: true,
      };
      ...
      return wb
        ...
        .then(() => {
          ...
          return wb2.xlsx.readFile(options.filename);
        })
        .then(wb2 => {
    ...
    it('serializes row styles and columns properly', () => {
      const filename = makeTestXlsxFileName();
      const options = {
        filename,
        useStyles: true,
      };
      ...
      return wb
        ...
        .then(() => {
          const wb2 = new ExcelJS.Workbook();
          return wb2.xlsx.readFile(filename);
        })
        .then(wb2 => {
    ...
    it('rich text', () => {
      const TEST_XLSX_FILE_NAME = makeTestXlsxFileName();
      const options = {
    ...
    it('with conditional formatting that contains numFmt (#1814)', async () => {
      ...
      cf2.rules.forEach(rule => {
        ...
        expect(typeof rule.style.numFmt.id).toBe('number');
        expect(typeof rule.style.numFmt.formatCode).toBe('string');
      });
    });
  });
});
